...
export default function AdminShell({
  ...
  React.useEffect(() => {
    if (status === "unauthenticated") {
      router.replace(`/login?redirect=${encodeURIComponent(pathname)}&reason=admin-auth`);
    }
  ...
  const handleSignOut = () => {
    signOut({ callbackUrl: "/login?reason=signed-out" });
  };
  ...
  const menuGroups = [
    {
      label: null,
      items: [{ text: "แดชบอร์ด", icon: <Category size="22" color="currentColor" />, path: "/admin" }],
    },
    {
      label: "จัดการร้านค้า",
      items: [
        { text: "จัดการสินค้า", icon: <Shop size="22" color="currentColor" />, path: "/admin/products" },
        { text: "คำสั่งซื้อ", icon: <Receipt1 size="22" color="currentColor" />, path: "/admin/orders" },
        { text: "คูปองส่วนลด", icon: <TicketDiscount size="22" color="currentColor" />, path: "/admin/coupons" },
        { text: "โปรโมชั่น", icon: <Tag size="22" color="currentColor" />, path: "/admin/promotions" },
    ...
    {
      label: "สมาชิก",
      items: [{ text: "จัดการผู้ใช้งาน", icon: <User size="22" color="currentColor" />, path: "/admin/users" }],
    },
    {
  ...
  const drawerContent = (
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <Box sx={{ p: 3, display: "flex", alignItems: "center", gap: 2, bgcolor: "white" }}>
        ...
        <Typography variant="overline" sx={{ fontWeight: 800, color: "primary.main", borderLeft: "1px solid #eee", pl: 1.5, lineHeight: 1 }}>
          Admin
        </Typography>
      </Box>
      ...
      <Box sx={{ flexGrow: 1, p: 2, overflowY: "auto" }}>
        {menuGroups.map((group, gi) => (
          <Box key={gi} sx={{ mb: 1 }}>
            {group.label && (
              <Typography variant="caption" sx={{ fontWeight: 800, color: "grey.400", letterSpacing: 1, px: 1.5, display: "block", mb: 0.5, textTransform: "uppercase" }}>
                {group.label}
              </Typography>
            )}
                  ...
                  <ListItem key={item.text} disablePadding>
                    ...
                    >
                      <ListItemIcon sx={{ color: "inherit", minWidth: 40 }}>
                        {React.cloneElement(item.icon as React.ReactElement<{ variant?: string }>, { variant: isActive ? "Bold" : "Linear" })}
                      </ListItemIcon>
      ...
      <Divider sx={{ borderColor: "grey.50" }} />
      <Box sx={{ p: 3 }}>
        <Button fullWidth variant="outlined" color="inherit" onClick={handleSignOut} startIcon={<LogoutCurve size="18" color="currentColor" />} sx={{ borderRadius: 3, py: 1, fontWeight: 700, borderColor: "grey.200", color: "grey.600" }}>
          ออกจากระบบ
        </Button>
      </Box>
    </Box>
        ...
        <Toolbar sx={{ justifyContent: "space-between", px: { xs: 2, lg: 4 } }}>
          ...
          <Stack direction="row" spacing={{ xs: 1, sm: 2 }} alignItems="center">
            ...
            <Box onClick={handleUserMenuOpen} sx={{ display: "flex", alignItems: "center", gap: 1.5, cursor: "pointer", p: 0.5, pr: 1.5, borderRadius: 10, transition: "all 0.2s", "&:hover": { bgcolor: "grey.50" } }}>
              <Avatar sx={{ bgcolor: "primary.main", width: 38, height: 38, boxShadow: "0 4px 10px rgba(215, 20, 20, 0.2)", fontSize: "0.9rem", fontWeight: 800 }}>
            ...
            >
              <MenuItem onClick={() => { handleUserMenuClose(); handleSignOut(); }} sx={{ borderRadius: 2, gap: 1.5, py: 1, color: "error.main" }}>
